...
const getProductFromDB = async (req, res, next) => {
  try {
    ...
    const result = await Product.find({
      "title": { $regex: regexProductName, $options: "i" },
    }).select("-categories -__v");
    if (result.length === 0) {
...
//     const result = await Product.find({
//       $or: [
//         { "title.ro": { $regex: regexProductName, $options: "i" } },
//         { "title.en": { $regex: regexProductName, $options: "i" } },
//       ],
//     }).select("-categories.ro -title.ro -__v");
//     if (result.length === 0) {
//       throw RequestError(404, "Not found");
//     }
//     res.json(result);
...
//       $or: [{ "title.ro": productName }, { "title.en": productName }],
//     });
//     if (product.length === 0) {
//       throw RequestError(500, "There is no such product in the database");
//     }
